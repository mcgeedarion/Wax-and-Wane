...
The Swift binary uses AVFoundation natively, integrates with Notification Center, and builds to a standalone executable.
...
1. **Webcam → ambient light**: Samples camera frames, extracts luma (Y plane), and averages across a rolling window.
...
The example configuration is `examples/config.json`. CLI flags override JSON config, and JSON config overrides built-in defaults.
...
| `smoothingWindow` | `5` | Rolling-average sample count; must be positive. |
| `changeThreshold` | `0.02` | Default brightness delta required before writing. |
| `riseThreshold` | `null` | Optional increase-specific threshold. |
| `fallThreshold` | `null` | Optional decrease-specific threshold. |
| `minUpdateIntervalSeconds` | `0.0` | Minimum seconds between backend writes. |
| `ambientDark` | `0.0` | Camera value representing room darkness. |
| `ambientBright` | `1.0` | Camera value representing room brightness. |
| `outputGamma` | `1.0` | Non-linear response curve. |
| `keyboardBackend` | `null` | Optional helper name (`kbrightness`, `mac-brightnessctl`). |
| `screenBackend` | `null` | Optional helper name (`brightness`, `ddcctl`). |
| `restoreOriginalBrightness` | `true` | Restore startup brightness where a backend can query it. |
| `dryRun` | `false` | Log intended backend writes without applying them. |

### Calibration workflow

1. Sit in a dim room and run `--dry-run`; note the raw ambient value from logs.
2. Set `ambientDark` to that dim-room value.
3. Sit in bright lighting and note the raw ambient value.
4. Set `ambientBright` to that bright-room value.
...
The Swift executable builds only the non-camera diagnostic/config path on Linux. The actual camera loop requires macOS AVFoundation, so CI runs Swift build/test on macOS. `make release` runs release-mode tests and CLI smoke checks; `make dist` creates a checksum file for the release binary.
